...
class Constr: # конструктор стен
    ...
    def __init__(self, x, y, width, height, width_one, height_one):
        self.wall = []
        ...
        for i in range(self.number_x):
            for j in range(self.number_y):
                block = Block(x + (self.width_one + 2) * i, y + (self.height_one + 2) * j, self.width_one, self.height_one)
...
# главный цикл
def gameplay():
    wall2 = Constr(30, 30, 500, 200, 50, 50)
    wall3 = Constr(WIDTH - 500, HEIGHT - 190, 500, 200, 50, 50)
    while 1:
